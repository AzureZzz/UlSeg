...
class ImageFolder_new(data.Dataset):
    def __init__(self, seg_list, GT_list, image_list, image_size=512, mode='train', augmentation_prob=0.4,
                 load_preseg=False):
...
class ImageFolder_new_difficult(data.Dataset):
    def __init__(self, seg_list, GT_list, image_list, load_preseg=False, image_size=512, mode='train',
    ...
    def __getitem__(self, index):
        ...
        Transform.append(T.Resize((final_size, final_size), interpolation=Image.BICUBIC))
        Transform_GT.append(T.Resize((final_size, final_size), interpolation=Image.NEAREST))
        Transform_seg.append(T.Resize((final_size*2, final_size*2), interpolation=Image.NEAREST))

        Transform.append(T.ToTensor())
        Transform_GT.append(T.ToTensor())
        Transform_seg.append(T.ToTensor())

        Transform = T.Compose(Transform)
        Transform_GT = T.Compose(Transform_GT)
        Transform_seg = T.Compose(Transform_seg)
