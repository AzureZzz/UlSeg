...
%% 参数
%�?��阈�?法卡掉crop黑边
value_thresold = 5;  % 因为是unit8型图像灰度�?0�?55，所以设�?0
...
%% 路径设置 set path
img_dir  = 'D:\DeskTop\stage1\image';
mask_dir = 'D:\DeskTop\stage1\mask';
csv_file = 'D:\DeskTop\stage1\origin.csv';
save_dir = 'D:\DeskTop\stage1\preprocessed';
...
%% 预处�
% 逐个处理，横向�?纵向裁剪掉黑�?抠出ROI，外�
for i = 1:length(filename_list)
    
    disp([num2str(i),'|',num2str(length(filename_list)),'|',filename_list{i}]);
